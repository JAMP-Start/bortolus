    ...
    const footerPromises = footer.data.body.map(async (slice: any, index: number) => {
      if (slice.slice_type === 'navigation') {
        const slicePromises = slice.items.map(async (nav: any) => {
          const navigation = await this.app.$prismic.api.getByUID('navigation', nav.navigation.uid, { lang })
          footer.data.body[index].primary = { ...navigation.data, id: nav.navigation.uid }
        })
